...
class LCA_CNN(torch.nn.Module):
    def __init__(self, in_size, channels, kernels, strides, fc, pools, unpools, paddings, activation=hard_sigmoid, softmax=False, lca=None, dict_loss="recon"):
        ...
        # ISSUE HERE WITH THE ARCHITECTURE THAT I HAVE WITH LCA, it does not get as small as size = 1 after the last pool, like normal P_CNN does
        # Thus, the hidden layer is really big (512, size, size) = 512, 3, 3
    ...
    def forward(self, x, y=0, neurons=None, T=29, beta=0.0, criterion=torch.nn.MSELoss(reduction='none'), check_thm=False, return_lca=False):
        ...
        for t in range(T):
            ...
            new_layers[0] = new_layers[0] + F.conv_transpose2d(unpools[0](layers[2], self.poolidxs[1]), self.synapses[1].weight, padding=self.paddings[1])
...
def evaluate(model, loader, T, device, return_lca=False):
    ...
    for x, y in loader:
        ...
        neurons = model.init_neurons(x.size(0), device)
        if return_lca:
            neurons, lca_acts, recon_errors = model(x, y, neurons, T) # dynamics for T time steps
