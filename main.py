...
parser.add_argument('--eps' ,type = float, default = 0.05, help= 'Adversarial Training Epsilon')
parser.add_argument('--image-print' ,type = int, default = 0, help= 'Print Attacked Images')
parser.add_argument('--attack-step',type = int, default = 29, help= 'Timesteps used to generate perturbation')
parser.add_argument('--predict-step',type = int, default = 30, help= 'Timesteps used to generate perturbation')
parser.add_argument('--train-attack-step' ,type = int, default = 10, help= 'Adversarial Training Attack Timesteps')
parser.add_argument('--attack-norm' ,type = int, default = 2, help= 'Attack Norm')

parser.add_argument('--n_feats',type = int, default = 784, metavar = 'df', help='Number of dictionary features.')
parser.add_argument('--lca_lambda',type = float, default = 0.25, metavar = 'll', help='LCA lambda.')
...
if args.task=='MNIST':
    ...
    mnist_dset_train = torchvision.datasets.MNIST('/storage/jr3548@drexel.edu/eplcanet/data/mnist_pytorch', train=True, transform=transform, target_transform=None, download=True)

    mnist_dset_test = torchvision.datasets.MNIST('/storage/jr3548@drexel.edu/eplcanet/data/mnist_pytorch', train=False, transform=transform, target_transform=None, download=True)
    ...
    test_loader = torch.utils.data.DataLoader(mnist_dset_test, batch_size=100, shuffle=False, num_workers=0)
    ...
    cifar10_train_dset = torchvision.datasets.CIFAR10('/storage/jr3548@drexel.edu/eplcanet/data/cifar10_pytorch', train=True, transform=transform_train, download=True)
    cifar10_test_dset = torchvision.datasets.CIFAR10('/storage/jr3548@drexel.edu/eplcanet/data/cifar10_pytorch', train=False, transform=transform_test, download=True)
...
if args.todo=='train':
    ...
    for idx in range(len(model.synapses)):
        if args.wds is None:
            if isinstance(model, nn.DataParallel):
                optim_params.append(  {'params': model.module.synapses[idx].parameters(), 'lr': args.lrs[idx]}  )
            else:
                optim_params.append(  {'params': model.synapses[idx].parameters(), 'lr': args.lrs[idx]}  )
        else:
            if isinstance(model, nn.DataParallel):
                optim_params.append(  {'params': model.module.synapses[idx].parameters(), 'lr': args.lrs[idx], 'weight_decay': args.wds[idx]}  )
            else:
    ...
    # training_acc /= len(train_loader.dataset)
    #print('\nTrain accuracy :', round(training_acc,2), file=open(path+'/hyperparameters.txt', 'a'))
    test_acc = attack(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax, args.image_print,args.attack_step, args.predict_step, args.attack_norm)
    test_acc /= len(test_loader.dataset)
    print(path,1)
    ...
    # training_acc /= len(train_loader.dataset)
    #print('\nTrain accuracy :', round(training_acc,2), file=open(path+'/hyperparameters.txt', 'a'))
    test_acc = img_print(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax, args.image_print,args.attack_step, args.predict_step, args.attack_norm)
    test_acc /= len(test_loader.dataset)
    print(path,1)
    #print('\nTest accuracy :', round(test_acc, 2), file=open(args.load_path+'/hyperparametersA.txt', 'a'))
    ...
    # training_acc /= len(train_loader.dataset)
    #print('\nTrain accuracy :', round(training_acc,2), file=open(path+'/hyperparameters.txt', 'a'))
    test_acc = attack_resume(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax, args.image_print,args.attack_step, args.predict_step, args.attack_norm)
    test_acc /= len(test_loader.dataset)
    print(path,1)
    #print('\nTest accuracy :', round(test_acc, 2), file=open(args.load_path+'/hyperparametersA.txt', 'a'))
elif args.todo=='hsj-attack':
    print('HSJ Attacking')
    test_acc = hsj_attack(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax, args.image_print,args.attack_step, args.predict_step,args.attack_norm)
    test_acc /= len(test_loader.dataset)
    print(path,1)
elif args.todo=='auto-attack':
    print('Auto Attacking')
    test_acc = auto_attack(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax, args.image_print,args.attack_step, args.predict_step,args.attack_norm)
    test_acc /= len(test_loader.dataset)
    print(path,1)
    ...
    # training_acc /= len(train_loader.dataset)
    #print('\nTrain accuracy :', round(training_acc,2), file=open(path+'/hyperparameters.txt', 'a'))
    baseline_print(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax)
    #print('\nTest accuracy :', round(test_acc, 2), file=open(args.load_path+'/hyperparametersA.txt', 'a'))
elif args.todo=='generate':
    print('Generating Adversarial Images')
    images = generate(model, test_loader, args.T1, criterion, device,args.load_path,args.softmax)
...
if args.todo == 'corruptions':
    ...
    test_acc = corruptions(model, test_loader, args.T1, device, args.load_path, args.softmax)
...
if args.todo=='advtrain':
    ...
    # Loading the state when resuming a run
    if args.load_path!='':
        checkpoint = torch.load(args.load_path + '/checkpoint_Train_Eps_%d.tar'%(int(args.eps*1000)))
        optimizer.load_state_dict(checkpoint['opt'])
        if checkpoint['scheduler'] is not None and args.lr_decay:
            scheduler.load_state_dict(checkpoint['scheduler'])
    else: 
    ...
    print('Success')
    adv_train(model, optimizer, train_loader, test_loader, args.T1, args.T2, betas, device, args.epochs, criterion, epsilon = args.eps, train_attack_step= args.train_attack_step, alg=args.alg, 
                 random_sign=args.random_sign, check_thm=args.check_thm, save=args.save, path=path, checkpoint=checkpoint, 
                 thirdphase=args.thirdphase, scheduler=scheduler, cep_debug=args.cep_debug)
