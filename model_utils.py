        ...
        if not self.softmax:
            ...
            for idx in range(conv_len):
                if isinstance(self.synapses[idx], LCAConv2D):
                    acts, recon_error = self.synapses[idx](layers[idx])
                else:
                    acts = self.synapses[idx](layers[idx])
                    
                phi = phi + torch.sum( (self.pools[idx](acts)) * layers[idx+1], dim=(1,2,3)).squeeze()     
    ...
    def forward(self, x, y=0, neurons=None, T=29, beta=0.0, criterion=torch.nn.MSELoss(reduction='none'), check_thm=False):
        ...
        for t in range(T):
            ...
            for idx in range(conv_len):
                ...
                # Apply synapse to layer activations and pool (result = new_layers)
                if isinstance(self.synapses[idx], LCAConv2D): 
                    lca_acts, recon_error = self.synapses[idx](layers[idx]) 
...
def check_gdu(model, x, y, T1, T2, betas, criterion, alg='EP'):
    ...
    if isinstance(model, EPLCANet):
        neurons, recon_errors, lca_acts = model(x, y, neurons, T1-T2, beta = beta_1, criterion=criterion)
    else:
    ...
    for K in range(T2+1):
        if isinstance(model, EPLCANet):
            neurons, recon_errors, lca_acts = model(x, y, neurons, K, beta=beta_1, criterion=criterion)
        else:
        ...
        for idx in range(len(neurons)):
            ...
            neurons[idx] = neurons[idx].detach() # added .clone to fix in-place error (JR)
            neurons[idx].requires_grad = True
            leaf_neurons.append(neurons[idx])
        if isinstance(model, EPLCANet):
            neurons, recon_errors, lca_acts = model(x, y, neurons, T2-K, beta=beta_1, criterion=criterion, check_thm=True)
        else:
    ...
    for t in range(T2):
        ...
        if isinstance(model, EPLCANet):
            neurons, recon_errors, lca_acts = model(x, y, neurons, 1, beta=beta_2, criterion=criterion)
        else:
    ...
    for epoch in range(epochs):
        ...
        for idx, (x, y) in enumerate(train_loader):
            ...
            if isinstance(model, EPLCANet):
                with torch.no_grad():
                    if isinstance(model, nn.DataParallel):
                        model.module.synapses[0].normalize_weights()
                    else:
                        model.synapses[0].normalize_weights()
            
            if isinstance(model, nn.DataParallel):
                neurons = model.module.init_neurons(x.size(0), device)
            else:
            ...
            if alg=='EP' or alg=='CEP':
                ...
                if isinstance(model, EPLCANet): # Update LCA weights using activations from first phase
                    if isinstance(model, nn.DataParallel):
                        model.module.synapses[0].update_weights(lca_acts, recon_errors)
                    else:
                        model.synapses[0].update_weights(lca_acts, recon_errors)
                
            elif alg=='BPTT':
                if isinstance(model, EPLCANet):
                    neurons, recon_errors, lca_acts = model(x, y, neurons, T1-T2, beta=0.0, criterion=criterion)           
                else:
                ...
                if isinstance(model, EPLCANet):
                    ...
                    # Do we not update LCA weights using activations from second phase?  
            ...
            if ((idx%(iter_per_epochs//10)==0) or (idx==iter_per_epochs-1)):
                ...
                if isinstance(model, EPLCANet):
                    print('Average LCA recon error ', (recon_errors ** 2).mean().item())
...
def attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print, attack_step, predict_step,attack_norm):
    ...
    for i in range(len(eps)):
        ...
        if criterion.__class__.__name__.find('MSE')!=-1:
            art_model = PyTorchClassifier(model, torch.nn.MSELoss(), (3, 32, 32), 100, predict_timesteps, attack_timesteps, last_layer=last_layer, clip_values=None,preprocessing=(mean,std))
        else:
            art_model = PyTorchClassifier(model, torch.nn.CrossEntropyLoss(), (3, 32, 32), 100, predict_timesteps, attack_timesteps, last_layer=last_layer, clip_values=None,preprocessing=(mean,std))
        ...
        if(image_print==0):
            ...
            x = torch.load('/storage/jr3548@drexel.edu/eplcanet/Equilibrium-Propagation/Test_Image.pt')[None,:,:,:].to(device)
            neurons = torch.from_numpy(art_model.predict(x.cpu().numpy())).to(device)
            x_adv = attack.generate(x.cpu().numpy())
            neurons_adv = torch.from_numpy(art_model.predict(x_adv)).to(device)
        ...
        if(image_print==0):
            ...
            if not last_layer:
                ...
                np.save(load_path + 'White_Labels_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%( int(eps[i]*1000),attack_timesteps,predict_timesteps),
                    labels)
                np.save(load_path + 'White_Norm_%d_Confidence_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%( int(eps[i]*1000),attack_timesteps,predict_timesteps),
                    confidence)
            else: 
...
def hsj_attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print, attack_step, predict_step,attack_norm):
    ...
    for i in range(3,len(test_eps)):
        ...
        print(loader.batch_size)
        if criterion.__class__.__name__.find('MSE')!=-1:
            art_model = PyTorchClassifier(model, torch.nn.MSELoss(), (3, 32, 32), 100, predict_timesteps, attack_timesteps, clip_values=None,preprocessing=(mean,std))
        else:
            
            base_model = BlackBoxClassifierNeuralNetwork(predict_square, input_shape=(3, 32, 32), nb_classes=100,clip_values=(0,1),preprocessing=(mean,std))
...
def auto_attack(model, loader, T, criterion, device,load_path,arg_softmax, image_print, attack_step, predict_step,attack_norm):
    ...
    for i in range(3,len(test_eps)):
        neurons = model.init_neurons(loader.batch_size, device)
        print(loader.batch_size)
        art_model = PyTorchClassifier(model, torch.nn.CrossEntropyLoss(), (3, 32, 32), 100, predict_timesteps, attack_timesteps, last_layer = last_layer, clip_values=(0,1),preprocessing=(mean,std))
        ...
        if(image_print==0):
            ...
            np.save(load_path + 'AA_Norm_%d_Square_Labels_Eps_%d_Max_Evals_%d.npy'%(norm_str,int(test_eps[i]*1000),int(max_evals[max_iter]*1000)),
                labels)
            np.save(load_path + 'AA_Square_Confidence_Eps_%d_Attack_T_%d_Predict_T_%d.npy'%(int(test_eps[i]*1000),attack_timesteps,predict_timesteps),
                confidence)
            acc = correct/run_total
            acc_adv = correct_adv/run_total 
            print(
